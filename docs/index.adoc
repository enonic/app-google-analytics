...
The report widget loads remote assets (scripts, styles, fonts and images) from Google servers and fetches
report data from the Google APIs. The app adds these hosts to Content Studio's Content Security Policy on
its own, so no CSP configuration is required.

NOTE: Versions before 5.0 required a `contentSecurityPolicy.header` line in
`{$xp_home}/config/com.enonic.app.contentstudio.cfg`. If you added one for this app, it is no longer
